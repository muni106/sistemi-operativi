...
{ 
	...
	while(1) {
		...
		DBGpthread_mutex_lock(&mutexLemuri, "lock lemuri");
		while( LemuriPossonoSalireSuIppopotamo == 0 || NLemuriSuIppopotamo >=4)
			DBGpthread_cond_wait(&condIppoEntraInAcquaRivaNord,&mutexLemuri,"salita lemuri");
		...
		DBGpthread_cond_wait(&condIppoEsceRivaSudProd, &mutexLemuri, "hihihahhhhh");
