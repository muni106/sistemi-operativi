...
{ 
	...
	while(1) {
		...
		/* pero' se c'e' qualche vecchietto che sta ancora leggendo
		...
		*/
		...
		while ( numVecchiettiInAttesa > 0){
			printf("l'attacchino deve aspettare\n");
		...
		/*  COMPLETARE */
		else DBGpthread_cond_signal(&condAttesaArrivoAttacchino2, Alabel);
		
		

		/* ORA SONO DUE GLI ATTACCHINI CHE COMINCIANO A LAVORARE */
		numAttacchiniInAttesa--;
		...
		/* quando entrambi gli attacchini hannosmesso di lavorare
		   l'ultimo sveglia i vecchietti per permettergli di
		   guardare i manifesti funebri ...  ALLEGRIAAAAA
		*/
		/*  COMPLETARE */
		DBGpthread_mutex_lock(&mutex, Alabel);
		
		numAttacchiniAlLavoro--;
		if(numAttacchiniAlLavoro == 0 && numAttacchiniInAttesa == 0 && numVecchiettiInAttesa > 0) {
	...
	while(1) {
		...
		*/	
		...
		if( numVecchiettiInLettura==0 &&  numAttacchiniInAttesa>0 ) 
			DBGpthread_cond_broadcast ( &condAttacchini, Vlabel );
		else if ( numVecchiettiInAttesa>0 )
			DBGpthread_cond_broadcast ( &condVecchietti, Vlabel );
